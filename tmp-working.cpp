...
void testDataWorkingMode(uint16_t device_id = 0xFFFF)
{
  ...
  if (workingMode == WorkingMode::mode_work)
  {
...
void setup()
{
  ...
  if (sds011.setDutyCycle(1))
  {
    Serial.println("SDS011 Duty Cycle set to 1min");
  }
  else
  {
...
{
  float p25, p10;
  if (sds011.queryData(p25, p10) == QueryError::no_error)
  {
